...
class Server:
    ...
    def get_hyper(self, page: int = 1, page_size: int = 10) -> List[List]:
        ...
        Returns;
            dict: Containing hypermedia
        """
        data = self.get_page(page, page_size)
        total_pages = - (len(self.__dataset) // - page_size)  # ceil division
        next_page = page + 1
        next_page = next_page if next_page <= len(self.__dataset) and \
            next_page <= total_pages else None
        prev_page = page - 1
